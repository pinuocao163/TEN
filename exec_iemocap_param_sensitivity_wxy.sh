...
PROMPTS="${PROJECT_DIR}/data/iemocap_llm_prompts.jsonl"
DEFAULT_CACHE="${DEFAULT_CACHE:-${PROJECT_DIR}/data_1/iemocap_Qwen2.5-7B-Instruct.jsonl}"
...
LOG_FILE="${LOG_FILE:-${RESULT_DIR}/iemocap_param_sensitivity_${SENS_GROUP}_${RUN_ID}.txt}"
CACHE_RUN_DIR="${SENS_CACHE_DIR}/iemocap_${RUN_ID}"
...
log_header() {
  cat >> "${LOG_FILE}" <<EOF
IEMOCAP parameter sensitivity experiments
time: $(date '+%F %T')
...
generate_cache() {
  ...
  local output_cache="${CACHE_RUN_DIR}/iemocap_${label}.jsonl"
  local limit_args=()
  if [[ -n "${LIMIT}" ]]; then
...
train_setting() {
  ...
  for iter in $(seq 1 "${RUNS}"); do
    {
      echo "============================================================"
      echo "Dataset=IEMOCAP Setting=${setting_name} Run=${iter}/${RUNS} Start=$(date '+%F %T')"
    ...
    echo "Dataset=IEMOCAP Setting=${setting_name} Run=${iter}/${RUNS} End=$(date '+%F %T')" >> "${LOG_FILE}"
  done
}

run_quality() {
  local settings=(
    "quality_q050_c080:0.50:0.80"
    "quality_q055_c0825:0.55:0.825"
    "quality_q060_c085:0.60:0.85"
    "quality_q065_c0875:0.65:0.875"
    "quality_q070_c090:0.70:0.90"
  )
...
run_rag_weight() {
  ...
  for weight in "${weights[@]}"; do
    ...
    train_setting "${label}" "${cache_file}" "0.60" "0.85"
  done
}
...
echo "Finished IEMOCAP parameter sensitivity experiments: $(date '+%F %T')" >> "${LOG_FILE}"
