...
本文对重新整理后的 LLM prompt 与 cache 文件进行统一统计。IEMOCAP 包含 7433 条 prompt，MELD 包含 13708 条 prompt；所有 cache 文件均可被 JSON 解析，解析失败数为 0。表 6 从覆盖数、平均置信度、平均 RAG 质量和标签一致性四个方面比较不同 LLM cache。这里的标签一致性用于衡量 LLM 输出标签与检索证据标签之间的匹配程度，反映 cache 是否受到训练集证据约束；它不是下游分类准确率，也不直接代表 LLM 的通用推理能力。
...
从覆盖率看，除 prompt 文件外，当前所有 IEMOCAP cache 均覆盖 7433 条样本，所有 MELD cache 均覆盖 13708 条样本，说明后续 LLM backbone 对比可以在完整 cache 上进行。覆盖率本身只说明 cache 是否可用，还不能说明 cache 是否可靠，因此需要同时观察置信度、RAG 质量和标签一致性。

同系列规模分析以标签一致性作为最核心依据，并结合平均 RAG 质量和平均置信度进行解释。按照标签一致性综合表现，本文将 Qwen 系列 cache 依次记为 Qwen2.5-7B、Qwen2.5-14B、Qwen2.5-3B 和 Qwen2.5-1.5B。Qwen2.5-7B-Instruct 在 IEMOCAP 上取得最高标签一致性 0.5711 和最高平均 RAG 质量 0.5816，说明其输出最能贴合训练集检索证据；Qwen2.5-14B-Instruct 在 IEMOCAP 上同样具有较高标签一致性 0.5595，并在 MELD 上达到 0.4557，整体稳定性次之；Qwen2.5-3B-Instruct 的 MELD 标签一致性为 0.4407，低于 14B，但仍保持较稳定的 RAG 质量；Qwen2.5-1.5B-Instruct 在 IEMOCAP 上标签一致性最低，仅为 0.4682，因此作为小模型效率对比。该排序强调的是 cache 与检索证据的一致程度，而不是单纯的模型参数量或置信度大小。

近参数跨模型分析进一步表明，不同 LLM 家族的 cache 风格差异明显。Mistral-7B-Instruct-v0.3 在 MELD 上平均置信度最高，达到 0.9079，同时保持 0.4367 的标签一致性，说明其输出更积极、覆盖更充分；InternLM3-8B-Instruct 在 IEMOCAP 上取得跨模型组最高 RAG 质量 0.5751 和最高标签一致性 0.5449，更贴合表演式双人对话的标签边界；Llama-3.1-8B-Instruct 在 IEMOCAP 上表现尚可，但在 MELD 上平均 RAG 质量降至 0.5322，标签一致性为 0.2932，说明开放多方场景对其当前 prompt 下的证据对齐更具挑战。总体来看，EACC 的关键并不是固定依赖某一个 LLM backbone，而是通过 RAG 质量、LLM 置信度和可靠性门控对不同来源的认知信息进行选择性吸收。
