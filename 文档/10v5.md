...
消融实验围绕 EACC 的三条核心路径展开：结构化认知变量是否有效、可靠性门控是否必要、VAD 情感几何是否改善易混淆类别。本文按表 5 组织逐项消融，其中 A0 到 A5 构成递进式模块验证，A6 和 A7 用于验证质量门控和校准强度是否能够抑制低质量认知噪声。为便于论文版式和后续实验替换，表 5 同时给出基于当前主实验和已完成敏感性实验估计的预期占位结果；占位值仅用于论文草稿，不作为最终实验结论。

表 5 消融实验配置与预期占位结果

| 编号 | 实验设置 | 训练改动 | IEMOCAP W-F1 | MELD W-F1 | 分析目的 |
| --- | --- | --- | --- | --- | --- |
| A0 | Base Encoder | 不使用 LLM cache，不启用 EACC | 73.35* | 66.35* | 验证基础三模态交互编码器性能 |
| A1 | + Structured Cognition | 使用 $c_i$，但不启用 LLM 蒸馏和可靠性监督 | 73.82* | 66.74* | 验证结构化认知变量是否补充多模态表示 |
| A2 | + LLM Distribution Alignment | 在 A1 基础上加入 $\mathcal{L}_{llm}$ | 74.18* | 67.05* | 验证 LLM 软分布是否提供类别关系信息 |
| A3 | + Reliability Alignment | 在 A2 基础上加入 $\mathcal{L}_{rel}$ | 74.43* | 67.31* | 验证模态提示和 VAD 是否能监督可靠性分配 |
| A4 | + Affective Geometry | 在 A3 基础上加入 $\mathcal{L}_{aff}$ | 74.68* | 67.48* | 验证 VAD 语义结构是否缓解易混淆类别 |
| A5 | Full EACC | 完整模型 | 74.99 | 67.62 | 报告本文最终性能 |
| A6 | w/o Quality Gate | 去掉 $\rho_i,\gamma_i$ 阈值过滤 | 74.12* | 66.91* | 验证低质量 LLM 推理是否引入噪声 |
| A7 | w/ Weak Calibration | 降低或固定 $\eta_i$ | 74.37* | 67.14* | 验证 EACC 的质量感知校准是否必要 |

注：带 * 的数值为预期占位结果，后续以实际运行日志替换；A5 使用表 3 的完整模型结果。

表 6 给出两组已经完成并能够从运行日志追溯的完整模型权重强度敏感性实验，可作为 A5 的稳定性证据。表中按每次运行的最佳 Weighted-F1 统计均值、标准差和最高值。结果显示，较小的认知损失权重在 IEMOCAP 和 MELD 上均略优，说明 EACC 更适合作为质量受控的辅助校准信号，而不应以过大的损失权重强行覆盖多模态编码器。
...
为保证消融与主实验可比，表 5 的逐项消融与表 6 使用相同的数据划分、训练轮数和评价流程。表 6 为已经完成并可从运行日志追溯的权重敏感性结果；表 5 中带 * 的逐项模块消融结果为占位值，用于明确控制变量和预期趋势，后续以实际运行结果替换。每个设置均应报告 Accuracy、Macro-F1、Weighted-F1 以及重复实验的均值与标准差；若版面有限，主表保留 Weighted-F1，附录补充完整分类报告和混淆矩阵。

## 4.5 LLM Backbone 与规模分析

为进一步验证 EACC 是否依赖特定 LLM，本文设置同一家族规模分析和跨模型 backbone 分析。根据已经生成的 cache，Qwen2.5-1.5B-Instruct、Qwen2.5-3B-Instruct、Qwen2.5-7B-Instruct 和 Qwen2.5-14B-Instruct 可直接用于 cache 质量比较。表 7 统计 cache 覆盖数、平均置信度、通过置信度阈值的比例、平均 RAG 质量和标签一致性。需要强调的是，表 7 衡量的是当前 prompt、RAG 检索和结构化解析规则下的 cache 可用性，而不是 LLM 通用能力排行，也不能直接替代下游训练结果。IEMOCAP 中 Qwen2.5-3B 在平均置信度、RAG 质量和标签一致性上均较优；MELD 中 3B 的置信度最高，14B cache 覆盖 4046 条样本，可作为部分样本上的大模型认知质量参考。

表 7 Qwen2.5 系列 cache 诊断统计
...
表 7 中 3B 指标高于 7B 是合理但需要谨慎解释的现象。EACC 训练阶段会通过质量阈值和置信度阈值筛选认知信息，因此 cache 的“可采用比例”会直接影响认知分支的有效样本数。当前 7B cache 的平均置信度更保守，尤其在 Conf. >= 0.90 的比例上明显低于 3B，导致更多样本无法通过高置信门控；而 3B 在当前生成设置下给出了更集中的置信度分布和更高的标签一致性，因此在 cache 诊断指标上更好。这一差异可能来自模型置信度校准、JSON 格式遵循、提示响应风格和当前 RAG 示例选择的共同作用，并不意味着 3B 的通用语言推理能力一定强于 7B。最终 backbone 选择仍应以下游 Accuracy、Macro-F1 和 Weighted-F1 为准。

基于表 7，训练阶段优先选择 Qwen2.5-3B 和 Qwen2.5-7B 进行完整性能对比：前者 cache 质量稳定且显存成本低，后者作为本文默认 LLM 能够代表更强规模的语言推理设置。14B 作为“大模型上限”实验，在 MELD cache 覆盖完整后再纳入下游训练比较。除同一家族规模分析外，本文进一步设置跨模型 backbone 扩展实验，如表 8 所示。由于 InternLM3-8B-Instruct、Mistral-7B-Instruct-v0.3 和 Llama-3.1-8B-Instruct 尚未完成 cache 生成和下游训练，表 8 中对应数值先以预期占位形式给出，后续以实际运行结果替换。三组模型运行时保持 RAG 参数、训练超参数、EACC 权重和评价指标完全一致，以检验 EACC 是否依赖特定 LLM 家族。

表 8 跨模型 backbone 补充实验预期结果

| LLM backbone | 参数规模 | 当前状态 | IEMOCAP Acc. | IEMOCAP W-F1 | MELD Acc. | MELD W-F1 | 对比目的 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Qwen2.5-7B-Instruct | 7B | 已完成 | 74.86 | 74.99 | 68.58 | 67.62 | 默认 backbone |
| InternLM3-8B-Instruct | 8B | 待运行，占位 | 74.40* | 74.55* | 68.15* | 67.35* | 检验中文指令模型在情感解释和结构化解析中的泛化能力 |
| Mistral-7B-Instruct-v0.3 | 7B | 待运行，占位 | 74.05* | 74.20* | 67.95* | 67.10* | 检验开源通用指令模型与 Qwen2.5-7B 同规模设置的差异 |
| Llama-3.1-8B-Instruct | 8B | 待运行，占位 | 74.20* | 74.35* | 68.05* | 67.20* | 检验强通用英文指令模型在跨语料 MERC 推理中的适配性 |

注：带 * 的数值为预期占位结果，后续以实际 cache 生成和下游训练结果替换。

## 4.6 RAG、VAD 与解释性分析

RAG 分析用于验证“训练集证据约束”是否真正提升 LLM 认知质量。本文设置三种检索形式：No-RAG 只向 LLM 输入当前话语和上下文；Text-RAG 只使用文本 TF-IDF 相似度检索；MM-RAG 使用文本相似度和音视频统计相似度联合检索。表 9 先给出预期占位结果，用于展示应报告的下游性能和 cache 质量；后续应使用对应 cache 和训练日志替换占位值。图 3 展示不同 RAG 质量分位区间中的 Weighted-F1 变化。

表 9 RAG 分析预期结果

| 设置 | 检索输入 | IEMOCAP W-F1 | MELD W-F1 | 平均 RAG 质量 | 标签一致性 | 预期现象 |
| --- | --- | --- | --- | --- | --- | --- |
| No-RAG | 不检索训练样本 | 74.05* | 66.85* | - | 0.50*/0.41* | 缺少训练集证据约束，LLM 推理更依赖自身先验 |
| Text-RAG | 文本上下文相似度 | 74.47* | 67.18* | 0.55* | 0.52*/0.43* | 文本相似案例提升情感标签一致性 |
| MM-RAG | 文本相似度 + 音视频统计相似度 | 74.99 | 67.62 | 0.57 | 0.54/0.44 | 多模态检索证据最充分，整体性能最高 |

注：带 * 的数值为预期占位结果；标签一致性按 IEMOCAP/MELD 顺序报告。MM-RAG 行使用当前完整模型和已生成 cache 的统计近似值。

VAD 分析用于验证连续情感空间是否改善相近类别的表示结构。本文设置两类消融：第一，去掉 $\mathcal{L}_{aff}$，即令 `vad-contrast-weight=0`；第二，屏蔽认知变量中的 appraisal 和 VAD guidance，仅保留 LLM 情感分布、模态提示、置信度和检索质量。表 10 先给出预期占位结果，用于展示整体性能、Macro-F1 和易混淆类别表现；后续应以实际运行结果替换。图 4 使用 t-SNE 或 UMAP 可视化 IEMOCAP 中 happy/excited、angry/frustrated、neutral/sad 三组类别的表示分布。

表 10 VAD 分析预期结果

| 设置 | 改动 | IEMOCAP W-F1 | MELD W-F1 | IEMOCAP Macro-F1 | MELD Macro-F1 | 预期现象 |
| --- | --- | --- | --- | --- | --- | --- |
| Full EACC | 完整 VAD appraisal、guidance 和 $\mathcal{L}_{aff}$ | 74.99 | 67.62 | 74.48 | 51.90 | 连续情感几何和质量门控共同提升整体识别 |
| w/o VAD Contrast | `vad-contrast-weight=0` | 74.54* | 67.28* | 73.95* | 51.35* | 易混淆情感对的边界变弱，Macro-F1 小幅下降 |
| w/o VAD Features | 屏蔽 appraisal 与 VAD guidance | 74.20* | 67.04* | 73.62* | 50.90* | 可靠性权重缺少情感几何先验，少数类和相近类受影响更明显 |

注：带 * 的数值为预期占位结果，后续以实际运行结果替换。
